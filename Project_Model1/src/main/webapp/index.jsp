    	...
    	<main id="main">
    		<div id="innerWrap" class="dFlex">
	    		...
	    		for(int i=0;i<noticeList.size();i++){
	    			AdmBoardBean list = noticeList.get(i);
	    			String postTM = list.getPostTM();
	    			String filename = list.getFilename();
	    			String indFile = "";
	    			if(filename!=null){
	    				indFile = "<img src='/images/clip_16x10.png' alt=''>";
					}
					long post = Long.parseLong(list.getPostTM().replaceAll("[^0-9]", ""));
					String noticeChk = "";
	    			...
	    			String type = "";
	    			if(list.getType()==1) type="필독";
	    			else type = "공지";%>
	    				<tr class="fixedNotice" onclick="Read(<%=list.getNum()%>,<%=nowPage%>)">
	    					<td><%=type %></td>
	    					<td><%=list.getSubject()+noticeChk+indFile %></td>
	    					<td><%=list.getadmName() %></td>
	    					<td><%=postTM %></td>
