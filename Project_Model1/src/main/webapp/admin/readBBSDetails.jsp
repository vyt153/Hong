		...
		<div id="bbsMng" class="contents">
	    		...
	    		%>
	    			...
	    			<table id="boardList">
	    				...
	    				<%
	    				vList = aMgr.getBoardList(uidParam, keyField, keyWord, start, end);
	    				listSize = vList.size();
	    				...
	    				<%if(uname.equals("관리자")) {%>
	    						...
	    						<td colspan="3">
	    							<form name="searchFrm" class="dFlex" id="searchFrm">
	    								...
	    								<div>
	    									...
	    									<input type="hidden" id="uid" value="<%=uidParam %>"/>
	    								</div>
	    								<div>
	    							...
	    							<input type="hidden" id="pUid" value="<%=uidParam %>"/>
	    						</td>
	    					</tr> <!-- tr#listBtnArea -->
	    					...
	    					if(totalPage != 0){
	    						if(nowBlock>1){%>
	    							<span id="moveBlockArea" onclick="moveBlock('<%=nowBlock-1%>','<%=pagePerBlock%>','<%=uidParam%>')">
	    							&lt;</span>
	    						<%} else{ %>
	    					...
	    					<% 
	    					 ...
	    					 <% } else{ %>
	    					 		<span class="PageNum" onclick="movePage('<%=pageStart %>','<%=uidParam%>')">
	    					 		<%=pageStart %></span>
	    					 <% }
	    					 } %>
	    					 <!-- 페이지 나누기용 페이지번호 출력 끝 -->
	    					 ...
	    					 <%if(totalBlock>nowBlock){//다음 블럭이 남아 있다면 %>
	    					 		<span class="moveBlockArea" onclick="moveBlock('<%=nowBlock+1 %>','<%=pagePerBlock %>',,'<%=uidParam%>')">
	    					 		&gt;</span>
	    					 <% } else{ %>
	    					 		<span class="moveBlockArea"></span>
	    					 <% }
