...
%>
<%@ page import="pack.csc.CscBean,java.util.Vector" %>
...
int numPerPage = 5; // 페이지당 출력하는 데이터 수
...
int end = 5; // 시작하는 인덱스 번호부터 반환하는 데이터 개수
...
totalRecord = cMgr.getTotalCount(keyField, keyWord);
   	...
   	<main id="main">
   		...
   		%>
   			...
   			<table id="boardList">
   				...
   				<%
   				vList = cMgr.getBoardList(keyField, keyWord, start, end);
   				listSize = vList.size();
   				...
   				%>
   				<%if(uname.equals("관리자")) {%>
   					<tr class="adminTr prnTr" onclick="admRead('<%=num%>', '<%=nowPage%>')">
   						<td>
   					...
   					<%} else{%>
   					<tr class="prnTr" onclick="admRead('<%=num%>', '<%=nowPage%>')">
   						<td>
   							<% if(depth==0) out.print(num);// 답변글이 아님을 의미함 %>
   						</td>
   						<td class="subjectTd">
   					...
   					if(totalPage != 0){
   						if(nowBlock>1){%>
   							<span id="moveBlockArea" onclick="admMoveBlock('<%=nowBlock-1%>','<%=pagePerBlock%>')">
   							&lt;</span>
   						<%} else{ %>
   					...
   					<% 
   					 ...
   					 <% } else{ %>
   					 		<span class="PageNum" onclick="admMovePage('<%=pageStart %>')">
   					 		<%=pageStart %></span>
   					 <% }
   					 } %>
   					 <!-- 페이지 나누기용 페이지번호 출력 끝 -->
   					 ...
   					 <%if(totalBlock>nowBlock){//다음 블럭이 남아 있다면 %>
   					 		<span class="moveBlockArea" onclick="admMoveBlock('<%=nowBlock+1 %>','<%=pagePerBlock %>')">
   					 		&gt;</span>
   					 <% } else{ %>
   					 		<span class="moveBlockArea"></span>
   					 <% }
