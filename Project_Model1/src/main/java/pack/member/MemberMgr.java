...
//회원 탈퇴 시작(/member/memberQuitProc.jsp)
	...
	public void delFile(String uid) {
		...
		try {
			...
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			path = "D:/AJR_20230126/Hong/silsp/p08_JSP/Project_Model1/src/main/webapp/fileupload";
			while(rs.next()) {
			...
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			path = "D:/AJR_20230126/Hong/silsp/p08_JSP/Project_Model1/src/main/webapp/galleryupload";
			while(rs.next()) {
				if(rs.getString("filename")!=null) {
					String fName = rs.getString("filename");
					String fileSrc = path + "/" + fName;
	...
	public String findId(MemberBean bean) {
		...
		try {
			sql = "select uid from member where uname = ?";
			sql += " and uemail = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, bean.getUname());
			pstmt.setString(2, bean.getUemail());
